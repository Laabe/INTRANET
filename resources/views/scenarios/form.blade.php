...
<p class="mb-4 text-14">{{ __('Workflow of the scenario') }}</p>

@foreach ($approvers as $index => $approver)
    <div class="row align-items-end">
        <div class="col-10 mb-3">
            <label for="approvers[{{ $index }}][id]"
                class="form-label">{{ __('Approver N° ') . $index + 1 }}</label>
...
@if ($count < 5)
    <div class="row">
        <div class="col-12 mb-3">
            <button class="btn btn-secondary" wire:click.prevent="addApprover">{{ __('Add approver') }}</button>
        </div>
    </div>
@endif
