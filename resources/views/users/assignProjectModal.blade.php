        ...
        <div class="modal-content modal-content-center">
            <form action="{{ route('users.assign-project', $user) }}" method="post">
                ...
                <div class="modal-header">
                    <h3 class="modal-title">{{ __('employeeManagement.Assign a project') }}</h3>
                    <button aria-label="Close" class="btn-close" data-bs-dismiss="modal">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <h5 class="mb-4">{{ __('employeeManagement.Select a project') }}</h5>
                    <div class="form-group mb-3">
                        <label for="project_id" class="form-label">{{ __('employeeManagement.project name') }}</label>
                        <select name="project_id" id="project_id"
                            class="form-control select2-show-search form-select select2-hidden-accessible @error('project_id') is-invalid @enderror"
                            data-placeholder="{{ __('employeeManagement.Select a project') }}">
                            <option value="">{{ __('employeeManagement.Select a project') }}</option>
