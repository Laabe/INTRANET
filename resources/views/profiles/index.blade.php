        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom justify-content-between">
                    ...
                    <a href="{{ route('profiles.create') }}" class="btn btn-primary">{{ __('profile.Add profile') }}</a>
                </div>
                <div class="card-body">
                    <div class="table-responsive export-table">
                        <table
                            ...
                            <thead>
                                <tr>
                                    <th>{{ __('profile.PROFILE NAME') }}</th>
                                    <th>{{ __('profile.TOTAL EMPLOYEES') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('profiles.edit', $profile) }}"
                                                class="btn btn-cyan">{{ __('profile.Edit') }}</a>
                                            <form class="d-inline-block" action="{{ route('profiles.destroy', $profile) }}"
                                                method="post">
                                                @csrf @method('delete')
                                                <button class="btn btn-danger" type="submit">{{ __('profile.Delete') }}</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
